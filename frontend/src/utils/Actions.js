...
export const setStartTime = (date) => (dispatch, getState) => {
  ...
  dispatch(
    updatingValidState(types.IS_CONFIG_VALID, isConfigValid && isInputValid),
  );
...
export const editExercise =
  (prevExercise, newExercise) => (dispatch, getState) => {
...
export const removeExercise = (numberInList) => (dispatch, getState) => {
  ...
  const newExercises = getState().config.exercises.filter(
    (_, i) => indexInExercises !== i,
  );
...
export const updateTime = (actionType, time) => (dispatch, getState) => {
  const key = actionType === types.UPDATE_WORK_TIME ? 'workTime' : 'restTime';
...
const validateConfig = (filteredConfig) => {
  return Object.keys(filteredConfig).reduce((acc, currKey) => {
    if (filteredConfig[currKey].length === 0 || !filteredConfig[currKey])
      acc = false;
